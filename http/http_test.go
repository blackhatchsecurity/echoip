...
import (
	...
	"github.com/blackhatchsecurity/echoip/iputil/geo"
)
...
func TestIPFromRequest(t *testing.T) {
	var tests = []struct {
		...
		{"127.0.0.1:9999", "X-Forwarded-For", "1.3.3.7", []string{"X-Forwarded-For"}, "1.3.3.7"},          // X-Forwarded-For with multiple entries (commas separator)
		{"127.0.0.1:9999", "X-Forwarded-For", "1.3.3.7", []string{"X-Forwarded-For"}, "1.3.3.7"},         // X-Forwarded-For with multiple entries (space+comma separator)
		...
		{"127.0.0.1:9999?ip=1.2.3.4", "X-Forwarded-For", "1.2.3.4", []string{"X-Forwarded-For"}, "1.2.3.4"}, // ip parameter wins over X-Forwarded-For with multiple entries
	}
	for _, tt := range tests {
